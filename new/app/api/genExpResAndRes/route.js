...
async function updateExpRes(promptId, completion) {
  debug('completed');
  const result = await sql`
    UPDATE
        prompts
...
async function updateRes(promptId, completion) {
  debug('completed');
  const result = await sql`
    UPDATE
        prompts
...
export async function POST(req) {
  ...
  const { promptData, col } = await req.json();
  console.log("promptData:", typeof(promptData), promptData)
  console.log("col:", col)
  const promptId = parseInt(promptData.id);
  // const promptId = await getPromptId(prompt, lng, maxTokens);
  let lng = promptData.lang
  ...
  if (completionRes) {
    ...
    if (col === "ExpRes") {
      await updateExpRes(promptId, completion);
    } else if (col === "Res") {
      await updateRes(promptId, completion);
    };
